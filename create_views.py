...
def transform_ir_ui_view(xml_file):
    ...
    new_root.append(etree.Element("field", name="name").text = record.findtext('field[@name="name"]'))
    new_root.append(etree.Element("field", name="priority").text = record.findtext('field[@name="priority"]'))
    new_root.append(etree.Element("field", name="type").text = "form")
